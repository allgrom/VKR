...
class Generate_Images_for_3D:
	def __init__(self, size_of_images, duration, path_to_save, file_with_label, video_file):
		...
		self.mean_r = np.zeros((720, 1280, 5))
		self.mean_g = np.zeros((720, 1280, 5))
		self.mean_b = np.zeros((720, 1280, 5))
	...
	def generate_empty(self, amount, amount_move):
		save = amount_move
		#compute background
		if self.frame >= 50:
			for i in range (0,5):
				self.mean_r[:,:,i] = self.video.get_frame((self.frame - i* 10) / self.video.fps)[:,:,0]
				self.mean_g[:,:,i] = self.video.get_frame((self.frame - i* 10)  / self.video.fps)[:,:,1]
				self.mean_b[:,:,i] = self.video.get_frame((self.frame - i* 10)  / self.video.fps)[:,:,2]
			...
			temp_image = (temp_image > 5) * 255
			temp_image = opening(temp_image, disk(2))
			# imsave("background/mean" + str(self.frame) + "__.png",temp_image)
			x1 = label(temp_image)
			t = regionprops(x1)
			shuffle(t)
			for region in t:
				...
				if y < 0:
					y = 0
					y_end = y + 100
				if not self.check_intersection([x,y, x_end,y_end]):
...
# generate_images_for_one_video(total_list[0])
pool = Pool(9)
