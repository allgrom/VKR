...
MODEL_FILE = 'deploy_smoke_fire.prototxt'
PRETRAINED = 'res_iter_12880_1.caffemodel'
...
transformer.set_mean('data', np.load('mean_2.npy').mean(1).mean(1)) # mean pixel
	...
	for frame in range(size * step + step, int(video.duration * video.fps), step):
		...
		for region in regionprops(image_label):
			...
			if (res_cnn == 2):
				...
				for i, n in enumerate(detect):
					...
					center_x = (x + x_end) / 2
					center_y = (y + y_end) / 2
					if (n[0][0] - cell_size <= center_y <= n[0][0] + cell_size) and (n[0][1] - cell_size <= center_x <= n[0][1] + cell_size) and n[2] != frame and n[3] == 2:
						n[1][0] = 1
						break
				else:
					detect.append((((y+y_end)/2, (x + x_end) /2), np.array([1, 0, 0, 0]), frame, 2))
				...
				for i, n in enumerate(detect):
					...
					center_x = (x + x_end) / 2
					center_y = (y + y_end) / 2
					if (n[0][0] - cell_size <= center_y <= n[0][0] + cell_size) and (n[0][1] - cell_size <= center_x <= n[0][1] + cell_size) and n[2] != frame and n[3] == 1:
						n[1][0] = 1
						break
				else:
					detect.append((((y+y_end)/2, (x + x_end) /2), np.array([1, 0, 0, 0]), frame, 1))
					# cv.rectangle(cur_image_with_rectangle, ((x + x_end) /2 - cell_size, (y+y_end)/2 - cell_size), ((x + x_end) /2 + cell_size, (y+y_end)/2 + cell_size), (0,0, 255), 2)
			else:
				color = (0, 255, 0)
		...
		for i, n in enumerate(detect):
			if (np.sum(n[1]) == 3 or np.sum(n[1]) == 4):
				...
				color = (0, 0, 0)
				if n[3] == 1:
					cv.rectangle(cur_image_with_rectangle,(n[0][1] - cell_size, n[0][0] - cell_size), (n[0][1] + cell_size, n[0][0] + cell_size), (0, 0, 255), 2)
				else:
